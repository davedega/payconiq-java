...
public class ExampleActivityTest {
    ...
    private RealmResults<RealmRepo> repos;
    ...
    public void setup() throws Exception {
        ...
        when(mockRealm.createObject(RealmRepo.class)).thenReturn(new RealmRepo());

        // Set up some naive stubs
        RealmRepo p1 = new RealmRepo();
        p1.setName("John Young");

        RealmRepo p2 = new RealmRepo();
        p2.setName("John Senior");

        RealmRepo p3 = new RealmRepo();
        p3.setName("Jane");

        RealmRepo p4 = new RealmRepo();
        p4.setName("Robert");

        List<RealmRepo> reposList = Arrays.asList(p1, p2, p3, p4);

        // Create a mock RealmQuery
        RealmQuery<RealmRepo> repoQuery = mockRealmQuery();

        // When the RealmQuery performs findFirst, return the first record in the list.
        when(repoQuery.findFirst()).thenReturn(reposList.get(0));

        // When the where clause is called on the Realm, return the mock query.
        when(mockRealm.where(RealmRepo.class)).thenReturn(repoQuery);

        // When the RealmQuery is filtered on any string and any integer, return the person query
        when(repoQuery.equalTo(anyString(), anyInt())).thenReturn(repoQuery);
        ...
        RealmResults<RealmRepo> repos = mockRealmResults();

        // When we ask Realm for all of the Person instances, return the mock RealmResults
        when(mockRealm.where(RealmRepo.class).findAll()).thenReturn(repos);
        ...
        when(repos.iterator()).thenReturn(reposList.iterator());

        // Return the size of the mock list.
        when(repos.size()).thenReturn(reposList.size());
    ...
    public void shouldBeAbleToAccessActivityAndVerifyRealmInteractions() {
        ...
        // Create activity
        PayconiqActivity activity = Robolectric.buildActivity(PayconiqActivity.class).create().start().resume().visible().get();
...
//        activity.findViewById(R.id.clean_up).sc();
